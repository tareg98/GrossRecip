...
fun AppNavGraph() {
    ...
    LaunchedEffect(Unit) {
        initialSession = sessionManager.currentSession()
    }

    // Still reading the saved session from disk - show nothing for a frame
    // rather than guessing whether the user is logged in.
    val startSession = initialSession ?: return

    val navController = rememberNavController()
    val startDestination = if (startSession.isLoggedIn) Routes.LISTS else Routes.LOGIN
